...
def segment():
    global output_image, mp_model, bg_image, num_cam,image_cap,result
    # For static images:
    IMAGE_FILES = []
...
def game():
   if game_placeholder.button('Start Game'):
        game_placeholder.empty()
        textgame_placeholder.write('เกมจะจับเวลาทุก 5 วินาที และถ่ายภาพ คุณต้องนั่งในท่านั่งที่ดีไปเรื่อยๆ')
        time_placeholder.write(countdown(5))
        ...
        while True:
            countdown(5)
            image_placeholder.image(image_cap)
            if result[0] == "00":
                st.balloons()
            if result[0] == "01":
